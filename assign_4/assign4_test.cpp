...
void rotate(node* child, node* parent) {
    if( child == parent->left) { // left child - right rotation
        node* y = child->right; // save for moving to parent's left-child
        child->right = parent; // L->right = GP;
        parent->left = y; // GP->left = LR
        if(y != nullptr) {
            y->parent = parent; // LR->parent = GP;
        }
        child->parent = parent->parent; // L->parent = GP->parent
        parent->parent = child; // GP->parent = L
    ...
    } else if (child == parent->right) { // right child - left rotation
        node* y = child->left;
        child->left = parent;
        parent->right = y;
        if(y != nullptr) {
...
node* insert(node* root, int value) {
    ...
    if(parent != nullptr) {
        left ? parent->left = current : parent->right = current;
...
node* splay(node* t) {
    // Keep performing splays until node t is the root of the tree
    node* parent;
    while(t->parent != nullptr) {
        parent = t->parent;
        
        // Check: is node paren't of root node?
        if( parent->parent == nullptr) {
            rotate(t, t->parent);
        } else { // grandparent is not null
            node* grandparent = parent->parent;
            if(
                // left-left or
                (grandparent->left == t->parent && parent->left == t) || 
                // right-right
                (grandparent->right == t->parent && parent->right == t)
                ) {
                ...
                // left-right or
                (grandparent->left == t->parent && parent->right == t) || 
                // right-left
                (grandparent->right == t->parent && parent->left == t)
            ) {
                rotate(t, t->parent);
                rotate(t, t->parent);
            }
...
int main() {
    ...
    //     if(t->parent->parent == nullptr) { // zig
    //         std::cout << "zig" << std::endl;
    //         rotate(t, t->parent);
    //     } else if ((t->key < parent->key && parent->key < grandparent->key) ||
    //                 (t->key > parent->key && parent->key > grandparent->key)) { // zig zig - outside-outside
    //                 std::cout << "zig zig" << std::endl;
    //                 rotate(parent, grandparent);
    //                 rotate(t, parent);
    //     } else if ((t->key < parent->key && parent->key > grandparent->key) ||
    //                 (t->key > parent->key && parent->key < grandparent->key)) { //zig zag - outside-inside/inside-outside
    //                 std::cout << "zig-zag" << std::endl;
    //                 rotate(t, parent);
    //                 std::cout << "t: " << t->key << " parent: " << parent->key << " grandparent: " << grandparent->key << std::endl;
    //                 rotate(t, parent);
    //                 std::cout << "t: " << t->key << " parent: " << parent->key << " grandparent: " << grandparent->key << std::endl;
    //     }
    // }